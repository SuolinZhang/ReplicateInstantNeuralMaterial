...
def train_epoch(model: nn.Module,
    ...
    for i, (x, y) in enumerate(loader, start=1):
        ...
        if do_step:
            ...
            opt.step()
            opt.zero_grad(set_to_none=True)
            # ---- QAT: step-based mode switch after calibration ----
            global_step += 1
                ...
                if hasattr(model, "set_quant_mode"):
                    try:
                        if int(qat_freeze_after) > 0:
                            ...
                            print(
                                f"[QAT] → 'quantize_observe' at step {global_step} (freeze after {int(qat_freeze_after)} steps).")
                        else:
                            model.set_quant_mode("quantize")
                            print(f"[QAT] → 'quantize' at step {global_step}.")
                    except AssertionError:
                        # model may not support 'quantize_observe'
                        model.set_quant_mode("quantize")
...
def main():
    ...
    ap.add_argument("--qat_freeze_after", type=int, default=0,
                    help = "Extra steps to keep observing while quantizing (quantize_observe) before freezing scales.")
    # ---- Export / reference ----
    ap.add_argument("--export_int8_dir", type=str, default=None,
                    help="If set, export Int8 weights and manifest to this directory after training.")
    ap.add_argument("--int8_ref_check", action="store_true",
                    help="Run a small CPU Int8 reference forward on a held-out mini-batch and report MAE.")
    ap.add_argument("--int8_ref_mae_max", type=float, default=1e-3,
                    help="Export parity gate: maximum allowed MAE between QAT and CPU-Int8.")
    ap.add_argument("--int8_ref_psnr_min", type=float, default=50.0,
                    help="Export parity gate: minimum allowed PSNR between QAT and CPU-Int8.")
    # ---- ACF synthesis (U-plane) ----
    ap.add_argument("--acf_k", type=int, default=0,
                    help="If >0, enable ACF-blended U-plane fetch with K offsets.")
    ap.add_argument("--acf_downsample", type=int, default=128,
                    help="Downsample size for ACF computation (speed).")
    ap.add_argument("--acf_update_every", type=int, default=1,
                    help="Recompute ACF every N epochs (1=recompute each epoch).")
    ...
    # Optionally enable ACF synthesis
    if args.acf_k > 0:
        model.enable_acf_synthesis(k=args.acf_k, downsample=args.acf_downsample)
    ...
    sched = CosineAnnealingWarmRestarts(opt, T_0=20, T_mult=2, eta_min=1e-5) # T_0=20, T_mult=2, eta_min=5e-5
  ...
  # Ensure validation reflects quantized behaviour if QAT is enabled
        prev_mode = None
        if args.qat and hasattr(model, "set_quant_mode"):
            prev_mode = "observe"  # default fallback
            # try to detect current mode from one QLinear (optional)
            try:
                for m in model.mlp:
        ...
        # refresh ACF offsets periodically (the U-plane evolves during training)
        if args.acf_k > 0 and ((ep == 1) or (ep % max(1, args.acf_update_every) == 0)):
            model.update_acf_from_uplane(k=args.acf_k, downsample=args.acf_downsample)
    ...
    if args.int8_ref_check:
        if args.qat and hasattr(model, "set_quant_mode"):
            model.set_quant_mode("quantize")
        # draw one mini-batch from the loader (fresh iterator)
        gate_bs = min(args.batch, 65536)
        it = iter(make_loader(args.btf, samples=gate_bs, batch=gate_bs,
                              workers=args.workers, reuse_per_frame=args.reuse_per_frame,
                              cache_size=args.cache_size))
